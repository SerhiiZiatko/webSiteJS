class ShopList{
    constructor() {
        ...
        // Для пагинации
        this.tempQuery = '';
        this.renderGoods(this.getGoods('', 2));
    ...
    async getGoods(query = '', page = '', saveTemp = true){
    ...
    async renderPagination(root){
        const count = Math.ceil(( await this.getGoods(this.tempQuery, '', false) ).length / 8) ;
        let pagination = `<ul class="pagination">`
        for(let i = 1; i <= count; i++){
            pagination += `<li><a data-number = '${i}' onClick = 'shopList.handlePageLink(event)' class = 'page-link' href = '#'>${i}</a></li>`
        }
        pagination += `</ul>`;
    ...
    async handlePageLink(e){
        ...
        this.renderGoods(await this.getGoods(this.tempQuery, pageNumber, false));
    }
    ...
    async renderGoods(goods){
        ...
        goods.forEach(item => {
            const good = `<li data-id = '${item.id}' class = 'good'>` +
                         ...
                         `<button onclick = 'shopList.addGoodToBasket(event)' class = 'button btn btn-danger'>Add to Basket</button>` +
                         `</li>`;
            list.innerHTML += good;
        });
        this.renderPagination(root, list.children.length);
    }
    ...
    async renderBasket(){
        ...
        for (let i = 0; i < localStorage.length; i++){
            ...
            const [good] = await this.getGoods(query, '', false);
            if(good){
    ...
    async handleFilters(){
        ...
        async function onFilter(e){
            ...
            let goods = await this.getGoods(query, 1);
